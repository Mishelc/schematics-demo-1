resource "ibm_is_lb" "lb1" {
  ...
  subnets = ["${ibm_is_subnet.public1.id}","${ibm_is_subnet.public2.id}"]
}
...
resource "ibm_is_lb_pool_member" "lb1-pool-member1" {
  ...
  port = "80"
  target_address = "${ibm_is_instance.appinstance1.primary_network_interface.0.primary_ipv4_address}"
}
...
resource "ibm_is_lb_pool_member" "lb1-pool-member2" {
  ...
  pool = "${ibm_is_lb_pool.lb1-pool.id}"
  port = "80"
  target_address = "${ibm_is_instance.appinstance2.primary_network_interface.0.primary_ipv4_address}"
}